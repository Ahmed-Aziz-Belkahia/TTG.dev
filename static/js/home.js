...
var PercentageArrowUP =
  '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2EBE7B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-move-up-right"><path d="M13 5H19V11"/><path d="M19 5L5 19"/></svg>';
var PercentageArrowDown =
  '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#DA5C54" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-move-down-right"><path d="M19 13V19H13"/><path d="M5 5L19 19"/></svg>';

console.log(
  "kyrix/zend: load crypto animation while fetching the data from backend"
);
    ...
    lossesPercentageBtcElement.forEach(function (element) {
      ...
      if (btc[1] > 0) {
        ...
        imgElement.setAttribute(
          "src",
          "data:image/svg+xml;base64," + btoa(PercentageArrowUP)
        );
        ...
        imgElement.setAttribute(
          "src",
          "data:image/svg+xml;base64," + btoa(PercentageArrowDown)
        );
        imgElement.setAttribute("width", "24");
        imgElement.setAttribute("height", "24");
  ...
  .addEventListener("click", function (event) {
    ...
    if (selectedValue) {
        ...
        function (response) {
          if (response.success) {
            document.querySelector(
              ".told-wrapper"
            ).innerHTML = `<div class="thank-you-message">
      ...
      popupSpan.textContent =
        "Please select an option before submitting your feedback.";
      popupMessage.style.display = "block";
      document
        .getElementById("ErrorPopUpCloseButton")
    ...
    ajaxRequest(
      ...
      function (response) {
        if (response.success) {
          ...
          var checkSVG =
            '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#2EBE7B" stroke-width="3" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-check"><path d="M20 6 9 17l-5-5"/></svg>';
          ...
          popupSpan.textContent =
            "You claimed 1000 points, get back the next day.";
          popupMessage.style.display = "block";
        } else {
          popupMessage.classList.remove("success");
          popupImage.src = "{% static 'assets/x-circle.svg' %}";
          popupSpan.textContent =
            "You already claimed your daily points! Try again tomorrow.";
          popupMessage.style.display = "block";
        }
      },
      function (error) {
...
function updateTrackInfo(name, image, description, banner) {
  ...
  if (trackDescriptionElement)
    trackDescriptionElement.textContent = description;
  if (trackBannerElement) {
    trackBannerElement.style.setProperty(
      "--banner-url",
      `url('/media/${banner}')`
    );
  } else {
